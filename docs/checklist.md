...
```
grep -n "このファイルは雛形として配布されている" docs/onboarding.md
```

何もヒットしないこと。ヒットしたら、`docs/onboarding.md` の雛形コメントと空欄（プロジェクトの説明、環境の立ち上げ方、相談先）がそのまま残っている。**参加者が最初に読むファイルなので、空欄のまま渡さない。**
